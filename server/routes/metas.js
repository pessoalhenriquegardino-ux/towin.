...
router.post('/', (req, res) => {
  const { titulo, descricao, foto_url, prazo, porque, categoria, valor_alvo, valor_atual } = req.body;
  if (!titulo || !porque) {
    return res.status(400).json({ erro: 'titulo e porque são obrigatórios' });
  }
  const info = db
    .prepare(
      `INSERT INTO metas (titulo, descricao, foto_url, prazo, porque, categoria, valor_alvo, valor_atual)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      titulo,
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(
    `UPDATE metas SET titulo=?, descricao=?, foto_url=?, prazo=?, porque=?, categoria=?, status=?, progresso=?,
     valor_alvo=?, valor_atual=?, ultima_atualizacao=datetime('now')
     WHERE id=?`
  ).run(
    atualizacoes.titulo,
